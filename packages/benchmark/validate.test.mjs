...
describe("validate output between libs", () => {
  test("slots false - twMerge no - compound yes", () => {
    ...
    ).toBe(CVA.noSlotsAndCompoundNoTwMergeNoResponsive.avatar({ size: "md" }));
    expect(
      CVA.noSlotsAndCompoundNoTwMergeNoResponsive.avatar({ size: "md" })
    ).toBe(
      TAILWINDBUDDY.noSlotsAndCompoundNoTwMergeNoResponsive.avatar.slots.root({
        size: "md",
      })
    );
  });
  ...
  test("slots false - twMerge yes - compound yes", () => {
    expect(TV.noSlotsCompound.avatar({ size: "md" })).toBe(
      twMerge(
        CVA.noSlotsAndCompoundNoTwMergeNoResponsive.avatar({ size: "md" })
      )
    );
